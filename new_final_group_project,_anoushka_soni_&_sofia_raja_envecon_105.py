...
# --- Choose the 10 countries (2010 emissions)
top_countries = (
    emissions_data_sorted[emissions_data_sorted['Year'] == 2010]
    .groupby('Country')['Value']
    .sum()
    .nlargest(10)
    .index
...
emissions_data_sorted = emissions_data_sorted[emissions_data_sorted['Country'].isin(top_countries)]
...
# Handle countries missing 2014 data
others = [c for c in top_countries if c not in order_2014]
legend_order = order_2014 + others

# --- Plot
plt.figure(figsize=(12, 6))
line_colors = {}  # store line colors to match legend text later
for country, g in emissions_data_sorted.groupby('Country'):
    ...
    if not g.empty:
        ...
        line_colors[country] = line.get_color()

# --- Labels, title, subtitle, legend
plt.title(
    "Top 10 Emissions-producing Countries in 2010 (1751-2014)\n"
    "Ordered by Emissions Produced in 2014",
    fontsize=16
)
plt.xlabel("Year", fontsize=12)
plt.ylabel("Emissions (Metric Tons)", fontsize=12)
plt.grid(True, linewidth=0.5, alpha=0.5)
...
# Color legend text to match line colors
for text in legend.get_texts():
    country = text.get_text()
    if country in line_colors:
        text.set_color(line_colors[country])
...
"""Now, lets try to make  a graph that has the country names attached to each line"""
...
top_emitters = (
    emissions_data_sorted.groupby('Country')['Value'].sum().nlargest(10).index
)
emissions_data_sorted = emissions_data_sorted[emissions_data_sorted['Country'].isin(top_emitters)]

plt.figure(figsize=(12, 6))
colors = plt.cm.tab10.colors

for i, (country, group) in enumerate(emissions_data_sorted.groupby('Country')):
    g = group.dropna(subset=['Year', 'Value']).sort_values('Year')
    if g.empty:
        continue  # nothing valid to plot/label

    plt.plot(g['Year'], g['Value'], color=colors[i % 10], linewidth=1.5)

    # label at the last valid year
    x_end = g['Year'].iloc[-1]
    y_end = g['Value'].iloc[-1]
    plt.text(x_end + 0.5, y_end, country, color=colors[i % 10],
             fontsize=10, va='center')

plt.title("Top 10 Emissions-producing Countries in 2010 (1800–2022)\n"
